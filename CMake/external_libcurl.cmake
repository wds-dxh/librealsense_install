# Check if we should build with update capability
if(CHECK_FOR_UPDATES)
    # Try to use system curl first if available
    find_package(CURL QUIET)

    if(CURL_FOUND)
        # Use system curl if available
        message(STATUS "Using system curl library")
        ...
        # Create a dummy custom target that satisfies the dependency without actual building
        add_custom_target(libcurl)
    else()
        # If system curl is not found, fall back to building from source
        pop_security_flags() # remove security flags

        include(ExternalProject)
        message(STATUS "Building libcurl from source")
    ...
    # If CHECK_FOR_UPDATES is disabled, try to find system curl
    find_package(CURL QUIET)
    if(CURL_FOUND)
        # Use system curl if available
        message(STATUS "Using system curl library")
        ...
        # If system curl is not found and updates are disabled, provide a minimal interface
        message(WARNING "libcurl not found and updates are disabled. Some features may not be available.")
        add_library(curl INTERFACE)
